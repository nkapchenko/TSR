...
def cmsflow(swo, dsc_curve, estim_curve, n, mr, payment_date):
    """
    Price CMS flow using swaption's calibration basket and call-put parity.
    ...
    * swo.get_annuity(dsc_curve) / dsc_curve.get_dsc(swo.expiry) for strike in strikes_ladders.caplet_ladder])
    ...
    * swo.get_annuity(dsc_curve) / dsc_curve.get_dsc(swo.expiry) for strike in strikes_ladders.floorlet_ladder])
    ...
    disc_Tf_Tp = dsc_curve.get_fwd_dsc(swo.expiry, payment_date)
    
    cms_swaplet = cms_caplet - cms_floorlet + swo.strike * disc_Tf_Tp
    adjustedCMS = cms_swaplet /  disc_Tf_Tp
    
    Cms_swaplet = namedtuple('Cms_swaplet', 'adjCMSrate swaplet caplet floorlet')
    return Cms_swaplet(adjustedCMS, cms_swaplet, cms_caplet, cms_floorlet)
